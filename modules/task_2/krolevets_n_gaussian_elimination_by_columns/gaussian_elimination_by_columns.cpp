// Copyright 2022 me
#include "../../../modules/task_2/krolevets_n_gaussian_elimination_by_columns/gaussian_elimination_by_columns.h"
...
void Gaussian_Solver::set_matrix(const std::vector<float>& vec, uint32_t x,
                                 uint32_t y) {
  assert(x == y + 1 && x > 0 && y > 0);
  orig_matrix = matrix = std::move(vec);
  size_x = x;
  size_y = y;
}
